...
lst_data = np.array(BD_complet[["x", "y", "s", "v", "HAUTEUR"]])
...
full_MAE_tot = []
for s in seeds:       
    lst_train, lst_test, id_train, id_test = train_test_split(lst_data, lst_id, test_size = 0.02, train_size = 0.98, random_state = s)
    ...
    lambda_values = [0, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1]
...
plt.figure()
# plt.plot(lambda_values, list_MAE_tot_1, color='lightblue')
# plt.plot(lambda_values, list_MAE_tot_2, color='lightblue')
# plt.plot(lambda_values, list_MAE_tot_3, color='lightblue')
# plt.plot(lambda_values, list_MAE_tot_4, color='lightblue')
# plt.plot(lambda_values, list_MAE_tot_5, color='lightblue')
plt.errorbar(lambda_values, list_MAE_tot_m, yerr=list_MAE_tot_s, color = "b", lw=2, label='Courbe moyenne')
# plt.errorbar(lambda_values, list_MAE_tot_s)
