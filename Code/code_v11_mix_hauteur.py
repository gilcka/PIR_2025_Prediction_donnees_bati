...
print("Suppression des bâtiments incomplets...")
...
neigh = NearestNeighbors(n_neighbors=n_voisins, algorithm='ball_tree', n_jobs=4)
...
for k in range(1, n_voisins):
    ...
    data_k = np.array(df_merged[['NATURE', 'USAGE1', 'LEGER', 'DATE_APP', 'HAUTEUR', 'SURFACE']])
    ...
    vect = np.hstack((vect, enc_nat_out, enc_us1_out, enc_leg_out, data_k[:, 3:], dist_k))
    
    for l in vect:
        date_app = l[-4]
        if date_app != None:
            l[-4] = int(date_app[0:4])
        else:
            l[-4] = 0
            
    columns_final += ['DATE_APP', 'HAUTEUR', 'SURFACE', 'DIST']
...
regressor = RandomForestRegressor(n_estimators=50, max_depth=20, n_jobs=4, criterion='absolute_error')
...
BD_complet_gdf.to_file("mix_paris_err_hauteur_n50_d20.gpkg") # export GPKG

plt.figure()
plt.hist(lst_reel, color='g', bins=[i for i in range(130)], edgecolor='k', alpha=0.8, label="Réalité")
plt.hist(lst_pred, bins=[i for i in range(130)], edgecolor='k', alpha=0.7, label="Prédiction")
plt.yscale('log')
